...
import { OllamaMessage } from "./mcp/types/ollamaTypes";
import { ToolManager } from "./mcp/ToolManager";
import { formatToolResponse } from "./mcp/utils/toolFormatters";
...
export class ChatManager {
    ...
    async initialize() {
        ...
        try {
            ...
            console.log("Ollama connection has been tested successfully");
        } catch (error) {
    ...
    async handleMessage(userInput: string): Promise<string> {
        ...
        try {
            try {
                ...
                const err = error as ErrorWithCause;
                if (err.cause?.code === "ECONNREFUSED") {
                    return "Error: Lost connection to Ollama server. Please ensure Ollama is running."
                } else {
                    return "Error processing input:" + (err instanceof Error ? err.message : String(err));
                }
            }
        } catch (error) {
